...
#            **config['optimizer'])
        ...
        if self.test_annotation:
            self.test_gen = self.data_gen('test_{}'.format(self.dataset_name), self.data_root_test, self.test_annotation, masked_language_model=False)

        # self.train_gen = self.data_gen_v1('train_{}'.format(self.dataset_name), self.data_root, self.train_annotation)
        # if self.valid_annotation:
        #     self.valid_gen = self.data_gen_v1('valid_{}'.format(self.dataset_name), 
        #             self.data_root, self.valid_annotation)
        # if self.test_annotation:
        #     self.test_gen = self.data_gen_v1('test_{}'.format(self.dataset_name), self.data_root_test, self.test_annotation)
    ...
    def train(self):
        ...
        for i in range(self.num_iters):
            ...
            if self.valid_annotation and self.iter % self.valid_every == 0:
                ...
                acc_full_seq, acc_per_char = self.precision('valid')
                ...
                if acc_full_seq < self.best_acc:
                    self.save_weights(self.export_weights + str(self.iter) + '.pth')
                    self.best_acc = acc_full_seq
        print('best WER:', self.best_acc)
...
#                loss = self.criterion(rearrange(outputs, 'b t v -> (b t) v'), rearrange(tgt_output, 'b o -> (b o)'))
    ...
    def predict(self, mode = 'valid'):
        ...
        img_files = []
        if mode == 'train':
          sample = self.train_gen
        elif mode == 'test':
          sample = self.test_gen
        else:
          sample = self.valid_gen
    ...
    def precision(self, mode='valid'):

        pred_sents, actual_sents, _, _ = self.predict(mode)
        print(pred_sents[:30])
        print(actual_sents[:30])
